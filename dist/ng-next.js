...
_dereq_("./integration/ES6PromiseHook");
...
},{"./decorators/Alias":2,"./decorators/Component":3,"./decorators/Config":4,"./decorators/Controller":5,"./decorators/Directive":6,"./decorators/Filter":7,"./decorators/Inject":8,"./decorators/MethodDecorators":9,"./decorators/Run":10,"./decorators/Self":11,"./decorators/Service":12,"./decorators/State":13,"./decorators/View":14,"./integration/ES6PromiseHook":15,"./integration/WatchIterableFix":16,"./util/Configuration":19}],2:[function(_dereq_,module,exports){
"use strict";
 ...
 * with async / await
 */
var nextAvailableDigest = new Date().getTime();
var scheduledDigest = null;
var $rootScope = null;
...
var requestDigestCycle = function requestDigestCycle() {
    if (!$injector) {
        $injector = (0, _AngularModuleResolver.angularInjector)();
    }
    ...
    if ($rootScope) {
        ...
        //Digest is available
        if (now >= nextAvailableDigest || !_Configuration.config.DEBOUNCE_DIGEST_MILLIS) {
            nextAvailableDigest = now + _Configuration.config.DEBOUNCE_DIGEST_MILLIS;
            digest();
        }
        //Digest must be delayed slightly
                ...
                if (scheduledDigest == null) {
                    scheduledDigest = setTimeout(function () {
                        scheduledDigest = null;
                        nextAvailableDigest = new Date().getTime() + _Configuration.config.DEBOUNCE_DIGEST_MILLIS;
                        digest();
...
var proxy = function proxy(fn) {
    return function () {
        ...
        if (_Configuration.config.ASYNC_AWAIT_ENABLED) {
            // Only request if enabled
            requestDigestCycle();
        }
        return result;
    };
...
var $$PromiseCatchOriginal = Promise.prototype.catch;
Promise.prototype.catch = function (error) {
    return $$PromiseCatchOriginal.call(this, proxy(error));
};

},{"../util/AngularModuleResolver":17,"../util/Configuration":19}],16:[function(_dereq_,module,exports){
"use strict";
...
},{"../util/AngularModuleResolver":17}],17:[function(_dereq_,module,exports){
"use strict";
 ...
 */
var NgNextConfig = (_NgNextConfig = {
    //Angular module
    MODULE: undefined

}, _defineProperty(_NgNextConfig, "MODULE", null), _defineProperty(_NgNextConfig, "DEBOUNCE_DIGEST_MILLIS", false), _defineProperty(_NgNextConfig, "ASYNC_AWAIT_ENABLED", true), _defineProperty(_NgNextConfig, "ALLOW_DECORATORS_IN_SERVICES", true), _defineProperty(_NgNextConfig, "STATE_DECORATOR", function STATE_DECORATOR(state) {
    return state;
}), _NgNextConfig);
